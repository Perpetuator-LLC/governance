...
- **module/ is generic, overlay/ is this organisation's.** Anything that names the organisation — a
  host, a domain, a network, a secret path — goes in `overlay/`; the module takes it as a declared
  input (`README.md` → *Inputs*) and must start with none set. The module lint enforces the split.
