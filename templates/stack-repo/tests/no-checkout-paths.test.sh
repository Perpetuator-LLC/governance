...
# A script derives the repository root from its own location instead:
...
#             rooted at $HOME or ~
# Exempt: the deploy vars files under deploy/ (the target dir is data there, by design), and a line
# carrying the marker `checkout-path: allow` with its reason (a path inside a container, say).
...
# scan TOP KIND REGEX — print one line per finding; 0 none · 1 found · 2 git grep failed
scan() {
  local out rc
  out="$(git -C "$1" grep -nIE -e "$3" -- . "$EXEMPT")"; rc=$?
  [ "$rc" -le 1 ] || { echo "git grep failed (exit $rc) on the $2 pattern" >&2; return 2; }
  out="$(printf '%s\n' "$out" | grep -vF -e "$ALLOW" | grep -v '^$')"
  [ -n "$out" ] || return 0
  printf '%s\n' "$out" | sed "s/^/$2  /"
  return 1
}
...
lint() {
  ...
  checkout_rx='(^|[^A-Za-z0-9_.~:/$}-]|\$HOME|\$[{]HOME[}]|~)/([A-Za-z0-9_.-]+/)*'"${name//./\\.}"'([^A-Za-z0-9_.-]|$)'
  scan "$top" home "$HOME_RX"; rc_home=$?
  scan "$top" checkout "$checkout_rx"; rc_checkout=$?
  if [ "$rc_home" = 2 ] || [ "$rc_checkout" = 2 ]; then return 2; fi
  [ "$rc_home" -eq 0 ] && [ "$rc_checkout" -eq 0 ]
}

# put REPO PATH LINE… — write the lines to REPO/PATH and track it
put() {
...
# shellcheck disable=SC2016  # the $ROOT/$HOME below are fixture TEXT, written unexpanded on purpose
{
  ...
  put "$NEG" "deploy/vars.example.$yml" "target_dir: /srv/stacks/$nn"
  put "$NEG" "services/web/compose.$yml" \
